...
- Run five initial synthetic live evaluation calls and at most one targeted rerun of a real failed behavior. They contain no user document, owner ID, token, secret, or retrieval operation.
...
At a conservative 4,000 input tokens plus the 256-token output ceiling, one call is about $0.00035 and the 100-call envelope is below $0.035 for generation. The six-call evaluation ceiling is below $0.003. Retrieval and other service dimensions remain separate; this estimate is not a billing guarantee.
